import { getStoredJsonValue, getStoredString, removeStoredKey, setStoredString } from "@/lib/storage"
import { ExpoOfflineLogger } from "@modules/expo-offline-logger"
...
const OFFLINE_LOGS_STORAGE_KEY = "seanime-server-offline-log-entries"
const OFFLINE_LOGGING_ENABLED_STORAGE_KEY = "seanime-server-offline-logging-enabled"
...
function redactSensitiveText(value: string): string {
    return value
        ...
        .replace(/\/Users\/.+?\/seanime-server-mobile\//g, "[sea-server]/")
        .replace(/\/home\/.+?\/seanime-server-mobile\//g, "[sea-server]/")
...
export function clearOfflineLogs() {
    ...
    ExpoOfflineLogger.clearLastProcessExit()
}
...
export async function getOfflineLogText(): Promise<string> {
    ...
    const lastProcessExit = await ExpoOfflineLogger.getLastProcessExit()
    ...
    if (lastProcessExit) {
        sections.push(["# Previous Android process exit", redactSensitiveText(lastProcessExit.trim())].join("\n"))
    }
